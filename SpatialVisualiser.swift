...
class RezSceneBuilder {
    ...
    private func makeWireframeBox(width w: CGFloat, height h: CGFloat, depth d: CGFloat, color: NSColor, edgeRadius: CGFloat = 0.035, depthOffset: CGFloat = 0) -> SCNNode {
        let node = SCNNode()
        ...
        // For all wireframe edges, use a slight polygon offset to avoid Z-fighting
        // Negative values bring geometry closer to camera
        // (SceneKit doesn't expose polygonOffset directly, so we rely on renderingOrder)
    ...
    private func layoutClasses(_ classes: [ParsedClass]) {
        ...
        for cls in classes {
            ...
            // No solid face panels — pure wireframe so you can always see inside
            ...
            for method in cls.methods {
                ...
                let methodBox = makeWireframeBox(width: methodRectW, height: methodRectH, depth: methodRectD,
                                                  color: methodColor, edgeRadius: 0.02, depthOffset: 2)
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var glowSlider: NSSlider?
    var glowSceneView: FlyThroughView?

    var currentBuilder: RezSceneBuilder?
    var mainWindow: NSWindow?

    func parseSwiftFiles(at paths: [String]) -> [ParsedClass] {
        var allClasses: [ParsedClass] = []
        for path in paths {
            let classes = parseSwiftFile(path)
    ...
    func showOpenPanel() -> [String]? {
        ...
        panel.title = "Open Swift Project"
        panel.message = "Select a .swift file or a folder containing Swift files"
        panel.canChooseFiles = true
        panel.canChooseDirectories = true
        panel.allowsMultipleSelection = true
        panel.allowedContentTypes = [.init(filenameExtension: "swift")!, .folder]
        ...
        for url in panel.urls {
            ...
            if isDir.boolValue {
                files.append(contentsOf: findSwiftFiles(in: url.path))
            } else {
    ...
    func loadProject(files: [String]) {
        let classes = parseSwiftFiles(at: files)
        print("📦 Found \(classes.count) types from \(files.count) files:")
        for c in classes {
        ...
        if let sv = glowSceneView {
            sv.scene = builder.scene
        }

        // Update window title
        let projectName = (files.first! as NSString).deletingLastPathComponent.components(separatedBy: "/").last ?? "Project"
        mainWindow?.title = "Spatial IDE — \(projectName) (\(files.count) files, \(classes.count) types)"

        // Apply current glow
        if let slider = glowSlider {
            glowSliderChanged(slider)
        }
    }

    @objc func openProject(_ sender: Any?) {
        guard let files = showOpenPanel() else { return }
        loadProject(files: files)
    }
    ...
    func setupMenuBar() {
        ...
        let appMenu = NSMenu()
        appMenu.addItem(withTitle: "Quit Spatial IDE", action: #selector(NSApplication.terminate(_:)), keyEquivalent: "q")
        appMenuItem.submenu = appMenu
        mainMenu.addItem(appMenuItem)

        let fileMenuItem = NSMenuItem()
        let fileMenu = NSMenu(title: "File")
        let openItem = NSMenuItem(title: "Open Project…", action: #selector(openProject(_:)), keyEquivalent: "o")
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if CommandLine.arguments.count > 1 {
            ...
            if isDir.boolValue {
                swiftFiles = findSwiftFiles(in: path)
            } else {
                swiftFiles = [path]
            }
        }

        // Always show open panel if no CLI argument
        if swiftFiles.isEmpty {
            if let files = showOpenPanel() {
                swiftFiles = files
            } else {
                // User cancelled — fall back to bundled Calculator
                if let resourcePath = Bundle.main.path(forResource: "Calculator", ofType: "swift") {
                    swiftFiles = [resourcePath]
                } else {
                    let execPath = CommandLine.arguments[0]
                    let scriptDir = (execPath as NSString).deletingLastPathComponent
                    let calcPath = scriptDir + "/Calculator.swift"
                    if FileManager.default.fileExists(atPath: calcPath) {
                        swiftFiles = [calcPath]
                    } else {
        ...
        let classes = parseSwiftFiles(at: swiftFiles)
        print("📦 Found \(classes.count) types from \(swiftFiles.count) files:")
        for c in classes {
            let kind = c.isProtocol ? "protocol" : (c.isEnum ? "enum" : "class")
            print("  \(kind) \(c.name): \(c.methods.count) methods, \(c.properties.count) properties")
            if let sup = c.superclass { print("    ↑ inherits: \(sup)") }
        ...
        sceneView = FlyThroughView(frame: frame, options: [SCNView.Option.preferredRenderingAPI.rawValue: SCNRenderingAPI.metal.rawValue])
        ...
        // Glow intensity slider overlay
        let sliderContainer = NSView(frame: NSRect(x: 0, y: 0, width: frame.width, height: 36))
        sliderContainer.autoresizingMask = [.width]
        sliderContainer.wantsLayer = true
        sliderContainer.layer?.backgroundColor = NSColor(red: 0.02, green: 0.02, blue: 0.06, alpha: 0.7).cgColor

        let sliderLabel = NSTextField(labelWithString: "Glow Intensity")
        sliderLabel.frame = NSRect(x: 12, y: 8, width: 110, height: 20)
        sliderLabel.font = NSFont(name: "Menlo", size: 10) ?? NSFont.systemFont(ofSize: 10)
        sliderLabel.textColor = NeonPalette.cyan.withAlphaComponent(0.8)
        sliderContainer.addSubview(sliderLabel)

        let slider = NSSlider(value: 0.0, minValue: 0.0, maxValue: 1.0, target: nil, action: nil)
        slider.frame = NSRect(x: 126, y: 8, width: 260, height: 20)
        slider.isContinuous = true
        slider.autoresizingMask = []
        slider.cell?.controlSize = .small
        sliderContainer.addSubview(slider)
        self.glowSlider = slider
        self.glowSceneView = sceneView

        slider.target = self
        slider.action = #selector(glowSliderChanged(_:))

        let mainContainer = NSView(frame: NSRect(x: 0, y: 0, width: frame.width, height: frame.height))
        mainContainer.autoresizingMask = [.width, .height]
        sceneView.frame = NSRect(x: 0, y: 36, width: frame.width, height: frame.height - 36)
        sceneView.autoresizingMask = [.width, .height]
        mainContainer.addSubview(sceneView)
        mainContainer.addSubview(sliderContainer)

        window.contentView = mainContainer
        self.mainWindow = window

        let projectName = (swiftFiles.first! as NSString).deletingLastPathComponent.components(separatedBy: "/").last ?? "Project"
        window.title = "Spatial IDE — \(projectName) (\(swiftFiles.count) files, \(classes.count) types)"
        ...
        // Apply initial glow=0
        glowSliderChanged(slider)
    }

    @objc func glowSliderChanged(_ sender: NSSlider) {
        let value = CGFloat(sender.doubleValue)
        guard let sv = glowSceneView, let scene = sv.scene else { return }

        // Update all emission intensities in the scene
        func updateEmissions(_ node: SCNNode) {
            if let geo = node.geometry {
                for mat in geo.materials {
                    if mat.emission.contents != nil {
                        mat.emission.intensity = value
                    }
                }
            }
            for child in node.childNodes {
                updateEmissions(child)
            }
        }
        updateEmissions(scene.rootNode)

        // Update bloom
        if let cam = sv.cameraNode?.camera {
            cam.bloomIntensity = value * 2.0
            cam.bloomThreshold = max(0.05, 0.5 - value * 0.5)
        }
    }
