...
struct ParsedMethod {
    let name: String
    let conditionals: [String]  // if, guard, switch
    let loops: [String]          // for, while, repeat
}
...
func parseSwiftFile(_ path: String) -> [ParsedClass] {
    ...
    for line in lines {
        ...
        for (prefix, isProt, isEn) in declPatterns {
            if trimmed.hasPrefix(prefix) && !trimmed.hasPrefix("//") {
                if let name = currentName {
                    if insideMethod {
                        currentMethods.append(ParsedMethod(name: methodName, conditionals: methodConditionals, loops: methodLoops))
                        insideMethod = false
                    }
        ...
        if insideType {
            ...
            // Parse conditionals and loops inside methods
            if insideMethod && !trimmed.hasPrefix("//") {
                if trimmed.hasPrefix("guard ") {
                    let cond = String(trimmed.prefix(30)).replacingOccurrences(of: " else {", with: "")
                        .replacingOccurrences(of: " else{", with: "")
                    methodConditionals.append(cond)
                } else if trimmed.hasPrefix("if ") {
                    let cond = String(trimmed.prefix(30)).replacingOccurrences(of: " {", with: "")
                    methodConditionals.append(cond)
                } else if trimmed.hasPrefix("switch ") {
                    let cond = String(trimmed.prefix(30)).replacingOccurrences(of: " {", with: "")
                    methodConditionals.append(cond)
                } else if trimmed.hasPrefix("for ") {
                    let loop = String(trimmed.prefix(30)).replacingOccurrences(of: " {", with: "")
            ...
            if braceDepth <= 0 && line.contains("}") && currentName != nil {
                ...
                // Method ended (back to class level)
                currentMethods.append(ParsedMethod(name: methodName, conditionals: methodConditionals, loops: methodLoops))
                insideMethod = false
            }
        }
...
class RezSceneBuilder {
    ...
    // MARK: - Pyramid (Tetrahedron) Builder
    ...
    private func layoutClasses(_ classes: [ParsedClass]) {
        ...
        for cls in classes {
            ...
            // Count total inner elements for sizing
            ...
            let methodRectW: CGFloat = cubeSize * 0.8
            let methodRectD: CGFloat = cubeSize * 0.4

            // Calculate method heights based on inner content
            ...
            for method in cls.methods {
                ...
                // --- Conditionals as PYRAMIDS inside this method ---
                ...
                // --- Loops as SPHERES inside this method ---
            ...
            // --- Properties as small diamond/octahedron shapes ---
...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        window.title = "⚡ Spatial IDE V3 — Rez Visualiser"
